...
public:
  ...
  void LoadDataString(String& output) {
  ...
  template <class Type>
  void LoadData(Type& output) {
    LoadData(&output);
  }
  template <class Type>
  void LoadData(Type* output, int32 array_size) {
    if (array_size <= 1) {
      LoadData(output);
    } else {
  ...
  template <class Type>
  void StructAddData(Type input, int16 size, string* datastring) {
    if (datastring)
  ...
  template <class Type>
  void StructAddData(Type input, int32 array_size, int16 size, string* datastring) {
    if (array_size > 0) {
      for (int32 i = 0; i < array_size; i++)
  ...
  template <class Type>
  void AddData(Type input, string* datastring = 0) {
    if (!datastring)
      datastring = &buffer;
    datastring->append((char*)&input, sizeof(input));
  }
  template <class Type>
  void AddData(Type input, int32 array_size, string* datastring = 0) {
    if (array_size > 0) {
      for (int32 i = 0; i < array_size; i++)
  ...
  template <class String>
  void AddDataString(String* input, string* datastring = 0) {
