...
public:
  ...
  Client* GetInactiveClientByCharID(int32 id) {
    lock_guard<mutex> guard(client_timeouts_mutex);

    for (const auto& kv : client_timeouts) {
      Client* client = kv.first.get();
  ...
  void AddClientTimeout(unique_ptr<Client> client, int timeout) {
    lock_guard<mutex> guard(client_timeouts_mutex);
    client_timeouts.insert(make_pair<unique_ptr<Client>, int32>(move(client), Timer::GetUnixTimeStamp() + timeout));
  }
