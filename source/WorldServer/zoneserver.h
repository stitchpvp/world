...
public:
  ...
  void SetInstanceType(int16 type) {
  ...
  int32 GetClientCount() { return clients.size() + incoming_clients.size(); }
  ...
  void RemoveFromSpawnRangeMap(Client* client);
  ...
  void SendSpawn(Spawn* spawn, Client* client);
  void SaveClients();

  bool SendRemoveSpawn(Client* client, Spawn* spawn, PacketStruct* packet = 0, bool delete_spawn = false);
  void AddSpawnUpdate(int32 spawn_id, bool info_changed, bool pos_changed, bool vis_changed, Client* client = nullptr);
  void RemoveSpawnUpdate(int32 spawn_id);

  void AddToSpawnRangeMap(Client* client);
...
private:
  ...
  void SetClientRangeDistance(Client* client, int32 spawn_id, float distance);
  float GetClientRangeDistance(Client* client, int32 spawn_id);
  bool HasClientRangeSpawn(Client* client, int32 spawn_id);
  map<int32, float>* GetClientRangeMap(Client* client);
  void RemoveFromClientRangeMap(Client* client, int32 spawn_id);
  ...
  /* Mutex Maps */
  MutexMap<Spawn*, Client*> client_spawn_map;
  ...
  map<Client*, unique_ptr<map<int32, float>>> spawn_range_map;

  map<Client*, mutex> client_range_mutex_map;
