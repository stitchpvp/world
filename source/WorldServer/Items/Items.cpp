...
void Item::serialize(PacketStruct* packet, bool show_name, Player* player, int16 packet_type, int8 subtype, bool loot_item){
	...
	if(show_name)
		packet->setSubstructSubstructDataByName("header", "info_header", "show_name", 1);
	if(packet_type == 0)
	...
	packet->setSubstructSubstructDataByName("header", "info_header", "packetsubtype", subtype);
	...
	if(item_stats.size() > 0){
		...
		for(int32 i=0;i<item_stats.size();i++){
			...
			if (client->GetVersion() >= 1028) {
				if (stat->stat_type == 6)
					packet->setArrayDataByName("value", stat->value, i);
				else
					packet->setArrayDataByName("value", (sint16)stat->value, i, 0U, true);
			}
			else
	...
	if(generic_info.adventure_default_level > 0){
		...
		packet->setSubstructDataByName("footer", "footer_unknown2", generic_info.adventure_default_level);
	}
	else{
		packet->setSubstructDataByName("footer", "required_level", generic_info.tradeskill_default_level);		
		packet->setSubstructDataByName("footer", "footer_unknown2", generic_info.tradeskill_default_level);
	}
	if(slot_data.size() > 0){
