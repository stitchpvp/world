...
void WorldDatabase::LoadBuyBacks(shared_ptr<Client> client) {
...
void WorldDatabase::SaveBuyBacks(shared_ptr<Client> client) 
{
...
void WorldDatabase::LoadCharacterActiveSpells(Player* player) {
	...
	while (result && (row = mysql_fetch_row(result))) {
		...
		if (lua_spell) {
			...
			spell_process->GetSpellTargets(lua_spell.get());
			spell_process->CastProcessedSpell(move(lua_spell), true);
...
void WorldDatabase::SavePlayerActiveSpells(shared_ptr<Client> client) {
	if (!client) return;
...
void WorldDatabase::DeleteCharacterActiveSpells(shared_ptr<Client> client, bool delete_all) {
	if (!client) {
		return;
	}
...
void WorldDatabase::SavePlayerSpells(shared_ptr<Client> client)
{
	if(!client)
		return;
...
int32 WorldDatabase::LoadAppearances(ZoneServer* zone, shared_ptr<Client> client){
	Query query, query2;
	MYSQL_ROW row;
...
bool WorldDatabase::LoadCharacterStats(int32 id, int32 account_id, shared_ptr<Client> client)
{
	DatabaseResult result;
	...
	if( database_new.Select(&result, "SELECT * FROM character_details WHERE char_id = %i LIMIT 0, 1", id) )
	{
		...
		while( result.Next() )
		{
			...
			client->GetPlayer()->SetFame(result.GetInt16Str("fame"));
...
bool WorldDatabase::loadCharacter(const char* ch_name, int32 account_id, shared_ptr<Client> client){
	Query query, query4;
	MYSQL_ROW row, row4;
	int32 id = 0;
...
void WorldDatabase::SaveCharacterSkills(shared_ptr<Client> client){
...
void WorldDatabase::SaveCharacterQuests(shared_ptr<Client> client){
	Query query;
...
void WorldDatabase::SaveCharRepeatableQuest(shared_ptr<Client> client, int32 quest_id, int16 quest_complete_count) {
...
void WorldDatabase::SaveCharacterQuestProgress(shared_ptr<Client> client, Quest* quest){
	Query query;
...
void WorldDatabase::LoadCharacterQuestProgress(shared_ptr<Client> client){
	Query query;
...
void WorldDatabase::LoadCharacterQuests(shared_ptr<Client> client){
...
void WorldDatabase::LoadSpawns(ZoneServer* zone)
{
	...
	npcs = ProcessSpawnLocations(zone, "SELECT sln.id, sle.id, slp.x, slp.y, slp.z, slp.x_offset, slp.y_offset, slp.z_offset, slp.heading, sle.spawn_id, sle.spawnpercentage, slp.respawn, slp.grid_id, slp.id, slp.expire_timer, slp.expire_offset, slp.pitch, slp.roll FROM spawn_location_placement slp, spawn_location_name sln, spawn_location_entry sle, spawn_npcs sn where sn.spawn_id = sle.spawn_id and sln.id = sle.spawn_location_id and sln.id = slp.spawn_location_id and slp.zone_id=%i ORDER BY sln.id, sle.id", SPAWN_ENTRY_TYPE_NPC);
	objects = ProcessSpawnLocations(zone, "SELECT sln.id, sle.id, slp.x, slp.y, slp.z, slp.x_offset, slp.y_offset, slp.z_offset, slp.heading, sle.spawn_id, sle.spawnpercentage, slp.respawn, slp.grid_id, slp.id, slp.expire_timer, slp.expire_offset, slp.pitch, slp.roll FROM spawn_location_placement slp, spawn_location_name sln, spawn_location_entry sle, spawn_objects so where so.spawn_id = sle.spawn_id and sln.id = sle.spawn_location_id and sln.id = slp.spawn_location_id and slp.zone_id=%i ORDER BY sln.id, sle.id", SPAWN_ENTRY_TYPE_OBJECT);
	widgets = ProcessSpawnLocations(zone, "SELECT sln.id, sle.id, slp.x, slp.y, slp.z, slp.x_offset, slp.y_offset, slp.z_offset, slp.heading, sle.spawn_id, sle.spawnpercentage, slp.respawn, slp.grid_id, slp.id, slp.expire_timer, slp.expire_offset, slp.pitch, slp.roll FROM spawn_location_placement slp, spawn_location_name sln, spawn_location_entry sle, spawn_widgets sw where sw.spawn_id = sle.spawn_id and sln.id = sle.spawn_location_id and sln.id = slp.spawn_location_id and slp.zone_id=%i ORDER BY sln.id, sle.id", SPAWN_ENTRY_TYPE_WIDGET);
	signs = ProcessSpawnLocations(zone, "SELECT sln.id, sle.id, slp.x, slp.y, slp.z, slp.x_offset, slp.y_offset, slp.z_offset, slp.heading, sle.spawn_id, sle.spawnpercentage, slp.respawn, slp.grid_id, slp.id, slp.expire_timer, slp.expire_offset, slp.pitch, slp.roll FROM spawn_location_placement slp, spawn_location_name sln, spawn_location_entry sle, spawn_signs ss where ss.spawn_id = sle.spawn_id and sln.id = sle.spawn_location_id and sln.id = slp.spawn_location_id and slp.zone_id=%i ORDER BY sln.id, sle.id", SPAWN_ENTRY_TYPE_SIGN);
	ground_spawns = ProcessSpawnLocations(zone, "SELECT sln.id, sle.id, slp.x, slp.y, slp.z, slp.x_offset, slp.y_offset, slp.z_offset, slp.heading, sle.spawn_id, sle.spawnpercentage, slp.respawn, slp.grid_id, slp.id, slp.expire_timer, slp.expire_offset, slp.pitch, slp.roll FROM spawn_location_placement slp, spawn_location_name sln, spawn_location_entry sle, spawn_ground sg where sg.spawn_id = sle.spawn_id and sln.id = sle.spawn_location_id and sln.id = slp.spawn_location_id and slp.zone_id=%i ORDER BY sln.id, sle.id", SPAWN_ENTRY_TYPE_GROUNDSPAWN);
...
void WorldDatabase::Save(shared_ptr<Client> client){
	Query query;
...
void WorldDatabase::SavePlayerFactions(shared_ptr<Client> client){
...
bool WorldDatabase::LoadPlayerFactions(shared_ptr<Client> client) {
...
void WorldDatabase::SavePlayerMail(shared_ptr<Client> client) {
	if (client) {
...
void WorldDatabase::LoadPlayerMail(shared_ptr<Client> client, bool new_only) {
...
int32 WorldDatabase::LoadPlayerSkillbar(shared_ptr<Client> client){
...
bool WorldDatabase::GetItemResultsToClient (shared_ptr<Client> client, const char* varSearch, int maxResults) {
	Query query;
	MYSQL_ROW row;
	int results = 0;
...
void WorldDatabase::ToggleCharacterOnline(shared_ptr<Client> client, int8 toggle) {
	if (client) {
		Query query;
		Player* player = client->GetPlayer();
		//if(!player->CheckPlayerInfo())
...
bool WorldDatabase::LoadCharacterInstances(shared_ptr<Client> client) 
{
	DatabaseResult result;
...
void WorldDatabase::ListLocations(shared_ptr<Client> client) {
	if (client) {
		Query query;
		MYSQL_ROW row;
...
void WorldDatabase::ListLocationPoints(shared_ptr<Client> client, int32 location_id) {
	if (client) {
		if (LocationExists(location_id)) {
			Query query;
...
void WorldDatabase::SaveCharPrefixIndex(sint16 index, int32 char_id, shared_ptr<Client> client){
	Query query;
...
void WorldDatabase::SaveCharSuffixIndex(sint16 index, int32 char_id, shared_ptr<Client> client){
	Query query;
...
void WorldDatabase::SaveCharacterCurrentLang(int32 id, int32 char_id, shared_ptr<Client> client)
{
	Query query;
