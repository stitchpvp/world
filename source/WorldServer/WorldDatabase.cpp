...
void WorldDatabase::LoadGroundSpawnEntries(ZoneServer* zone) {
  ...
  while (result && (row = mysql_fetch_row(result))) {
    ...
    LogWrite(GROUNDSPAWN__DEBUG, 5, "GSpawn", "---Loading GroundSpawn ID: %u\n"
                                              ...
                                              "---harvest_coin: %u",
             atoul(row[0]), atoi(row[1]), atoi(row[2]), atoi(row[3]), atof(row[4]), atof(row[5]), atof(row[6]), atof(row[7]), atof(row[8]), atof(row[9]), atoul(row[10]));
...
void WorldDatabase::LoadGroundSpawnEntry(ZoneServer* zone, int32 entry_id) {
  ...
  if (result.GetNumRows() > 0 && result.Next()) {
    ...
    LogWrite(GROUNDSPAWN__DEBUG, 5, "GSpawn", "---Loading GroundSpawn ID: %u\n"
                                              ...
                                              "---harvest_imbue: %.2f, harvest_rare: %.2f, harvest10: %.2f\n"
                                              "---harvest_coin: %u",
             entry_id, result.GetInt16(0), result.GetInt16(1), result.GetInt8(2), result.GetFloat(3), result.GetFloat(4), result.GetFloat(5), result.GetFloat(6), result.GetFloat(7), result.GetFloat(8), result.GetInt32(9));
